 ...
 *
 * SVN $Revision: 10095 $
 * SVN $Date: 2018-05-31 19:31:01 +0530 (Thu, 31 May 2018) $
 */
/*=========================================================================*//**
  ...
  PDMA channel has been configured. Or can be resumed by a call to the 
  PDMA_resume() function if previously configured using PDMA_start_paused() 
  function. Once started, further data can be pushed through the PDMA channel by
  calling the PDMA_load_next_buffer() function. The PDMA_load_next_buffer() 
  function can be called every time a call to the PDMA_status() function indicates
  that the PDMA channel used for the transfer has a free buffer or it can be called
  as a result of a PDMA interrupt.
...
/***************************************************************************//**
  The PDMA_start_paused() function configures the DMA driver for next transfer. 
  It specifies the source and destination address of the transfer as well as the
  number of transfers that must take place. The source and destination addresses
  can be the address of peripheral registers. The transfer can later be initiated
  by using the PDMA_resume() function.
